...
class load_dialog():
    def __init__(self, root=None, max=280) -> None:
        ...
        self.window.grid()
        self.bar = ttk.Progressbar(self.window, orient='horizontal', mode='determinate', length=280)
...
class SettingsDialog(tk.Toplevel):
    def __init__(self, root, settings: Settings, page=0) -> None:
        ...
        self.confirm = {'frame': ttk.Frame(self)}
    ...
    def createPaths(self):
        def create_row(
            ...
            row = 0,
            options : list[str] = []
        ) -> dict[typing.Literal['label', 'var', 'entry', 'button']]:
            ...
            def get_entry(type : typing.Literal['text', 'options'], var, options : list = []):
                if type == 'options':
                    return ttk.Combobox(parent, textvariable = var, values = options)
                else:
                    return ttk.Entry(parent, textvariable = var)

            entry = get_entry(entry_type, var = var, options = options)
            ...
            if use_button:
                button = ttk.Button(parent, text = button_text, command = lambda *args : var.set(button_callback(var.get())))
                button.grid(row = row, column = 2, sticky = 'ew', padx=4, pady=2)

            return {'label' : label, 'var' : var, 'entry' : entry, 'button' : button}

        def validate(default = '', result = None):
            if result in ['', None]:
                return default
            else:
                return result
        ...
        self.paths['contents']['game']['contents']['gamepath'] = create_row(
            ...
            button_callback = lambda path : validate(path, filedialog.askdirectory(initialdir = os.path.dirname(path), title = 'Game directory')),
            row = 0
        )
        ...
        def getFile(path : str):
            ...
            assets = os.path.abspath(wmwpy.utils.path.joinPath(self.settings.get('game.gamepath'), self.settings.get('game.assets')))
...
class _AskStringOptions(simpledialog.Dialog):
    ...
    def body(self, master: tk.Frame) -> tk.Misc | None:
        if isinstance(self.prompt, str):
            prompt = ttk.Label(master, text = self.prompt)

            prompt.pack(side = 'top', anchor = 'center')

        if len(self.options) > 0:
            self.entry = ttk.Combobox(master, values = self.options)
        else:
        ...
        self.validate_label = ttk.Label(master, text = self.validate_message if isinstance(self.validate_message, str) else 'Error', style = 'Validation.TLabel', anchor = 'nw')
...
def askstringoptions(parent = None, title = None, prompt : str = None, validate_message : str = None, options : list[str] = [], validate_callback : typing.Callable[[str], bool] = None, **kwargs):
    d = _AskStringOptions(parent = parent, title = title, prompt = prompt, validate_message = validate_message, options = options, validate_callback = validate_callback, **kwargs)
...
if __name__ == '__main__':
    ...
    if test == 'about':
        ...
        app = tk.Tk()
        dialog = SettingsDialog(app, settings = settings)